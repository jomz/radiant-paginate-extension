module PaginateTags
  ...
  tag 'paginate' do |tag|
    ...
    paginated_children = Page.paginate(options.merge(:conditions => ["pages.parent_id in (?) AND virtual = ? and status_id = ? and published_at <= ?", parents, false, 100, Time.current], :order => 'published_at DESC'))
